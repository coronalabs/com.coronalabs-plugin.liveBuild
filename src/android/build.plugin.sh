...
# 1) Provide the path as a command line argument. For example:  build.sh <MyAndroidSdkPath>
...
if [ -z "$CORONA_ENTERPRISE_DIR" ]; then
...
PLUGIN_NAME="plugin.liveBuild"

## Clean
ant -buildfile build.plugin.xml -D"manifest.file=$ANDROID_MANIFEST" -D"CoronaEnterpriseDir=$CORONA_PATH" clean
checkError

## Compile Jar (outputs to bin/$PLUGIN_NAME.jar)
ant -buildfile build.plugin.xml -D"manifest.file=$ANDROID_MANIFEST" -D"CoronaEnterpriseDir=$CORONA_PATH" -D"CoronaPluginName=$PLUGIN_NAME" release
checkError
...
if [ "$OUTPUT_PLUGIN_DIR_ANDROID" ]; then
